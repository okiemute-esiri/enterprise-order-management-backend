...
export function createApp(service: OrderManagementService = createDefaultService()) {
  ...
  app.post("/api/v1/customers", async (req, res, next) => {
    try {
      ...
      res.status(201).json({ data: await service.createCustomer(input) });
  ...
  app.post("/api/v1/products", async (req, res, next) => {
    try {
      ...
      res.status(201).json({ data: await service.createProduct(input) });
    } catch (error) { next(error); }
  });

  app.post("/api/v1/inventory/:productId/adjustments", async (req, res, next) => {
    try {
      const { quantity } = inventorySchema.parse(req.body);
      res.json({ data: await service.adjustInventory(req.params.productId, quantity) });
    } catch (error) { next(error); }
  });
  ...
  app.post("/api/v1/orders", async (req, res, next) => {
    try {
      ...
      res.status(201).json({ data: await service.createOrder(input) });
    } catch (error) { next(error); }
  });

  app.post("/api/v1/orders/:orderId/confirm", async (req, res, next) => {
    try { res.json({ data: await service.confirmOrder(req.params.orderId) }); }
    catch (error) { next(error); }
  });

  app.post("/api/v1/orders/:orderId/cancel", async (req, res, next) => {
    try { res.json({ data: await service.cancelOrder(req.params.orderId) }); }
    catch (error) { next(error); }
  });
